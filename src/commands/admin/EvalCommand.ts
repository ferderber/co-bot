...
export default class EvalCommand extends Command {
  ...
  public async run(msg: Message, args: Map<string, unknown>): Promise<string> {
    ...
    const embed = new MessageEmbed({
      ...
      description: `Result: ${result}\nEval'd in \
            ${
              Math.round((diff[0] * NS_PER_SEC + diff[1]) * MS_PER_NS) / 1000
            } s`,
    });
    embed.setAuthor(msg.author.username, msg?.author?.avatarURL() ?? undefined);
