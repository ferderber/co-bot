...
export default class LeagueLookupCommand extends Command {
  ...
  public async run(msg: Message): Promise<void> {
    ...
    //     if (summoner) {
    //       const matchList = (
    //         await this.api.Match.gettingListByAccount(summoner.accountId)
    //       ).matches;
    ...
    //         const game = games[i];
    //         const participant = this.getParticipantByAccountId(
    //           game,
    //           summoner.accountId
    //         );
    //         recordStr += participant.stats.win
    //           ? (wins++, "W")
    //           : (losses++, "L");
    //         if (i !== 0) {
    //           recordStr += "-";
  ...
  //   if (!this.champions) {
  //       return (
  //         await this.api.StaticData.gettingChampions({
  //           dataById: true,
  //         })
  //       ).data
  //   }
  ...
  //   });
  //   const champList = [...champMap.values()].sort(
  //     (champA, champB) => champB.numPlayed - champA.numPlayed
  //   );
  //   return champList;
  // }
  ...
  //     (participant: any) => participant.player.accountId === accountId
  //   ).participantId;
