    ...
    async run(msg, args) {
        ...
        if (voiceChannel) {
            if (/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)$/.test(args.sound)) {
                if (bot.voiceConnection == null || bot.voiceConnection.voiceChannel.id != voiceChannel.id) {
                ...
                await Sounds.findOne({
                    ...
                    if (sound) {
                        ...
                        if (bot.voiceConnections.first() === null || bot.voiceConnections.first().voiceChannel.id != voiceChannel.id)
                            return voiceChannel.join().then((con) => {
